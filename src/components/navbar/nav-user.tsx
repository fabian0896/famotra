...
export function NavUser() {
  ...
  const { avatar } = useAvatar(auth?.profile.name);
          ...
          <DropdownMenuTrigger asChild>
            ...
            >
              <Avatar className="h-8 w-8 rounded-lg">
                <AvatarImage src={avatar} alt={auth?.profile.email} />
                <AvatarFallback className="rounded-lg">CN</AvatarFallback>
              </Avatar>
              <div className="grid flex-1 text-left text-sm leading-tight">
                <span className="truncate font-medium">{auth?.profile.name}</span>
                <span className="truncate text-xs">{auth?.profile.email}</span>
              </div>
